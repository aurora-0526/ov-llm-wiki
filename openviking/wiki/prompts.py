...
def build_bottom_node_discovery_prompt(
    ...
    prompt = _render_wiki_prompt("wiki.bottom_node_discovery", inputs)
    return prompt + "\n\nImportant: discover stable semantic topics, not arbitrary keyword buckets. Documents may belong to multiple Wiki topics; do not force mutually exclusive clusters."
...
def build_parent_node_discovery_prompt(
    ...
    prompt = _render_wiki_prompt("wiki.parent_node_discovery", inputs)
    return prompt + "\n\nImportant: construct a multi-parent DAG. The same child topic may be listed under multiple parent topics when each parent expresses genuine semantic containment. Do not force single-parent assignments."
