...
class SourceRefBuilder:
    ...
    def build_document_refs_by_node(
        ...
        for item in assignments:
            corrected_source_ids = [_resolve_source_id(source_id, cards_by_id) for source_id in item.source_ids]
            unknown_source_ids = [source_id for source_id in corrected_source_ids if source_id not in cards_by_id]
            if unknown_source_ids:
                logger.warning("assignment for node %s references unknown doc_id values; ignoring them: %s", item.node_id, unknown_source_ids)
            known_source_ids = list(dict.fromkeys(source_id for source_id in corrected_source_ids if source_id in cards_by_id))
...
def _resolve_source_id(source_id: str, cards_by_id: dict[str, DocumentCard]) -> str:
    if source_id in cards_by_id:
        return source_id
    candidates = [
        candidate for candidate in cards_by_id
        if len(candidate) == len(source_id)
        and SequenceMatcher(None, source_id, candidate).ratio() >= 0.9
    ]
    return candidates[0] if len(candidates) == 1 else source_id
